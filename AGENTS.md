...
1. **Keep one Vercel endpoint.** `api/mcp.ts` is the single Streamable HTTP endpoint. Do not split tool domains into modules without an explicit architecture change.
...
5. **Keep the tested MCP dependency pair exact.** `mcp-handler@1.1.0` and `@modelcontextprotocol/sdk@1.26.0` are a compatibility pair. Never independently float either version.
6. **Use package.json as the version source.** MCP `serverInfo.version` derives from `package.json`; never add another hardcoded server version.
...
| `.cursor/rules/repository-harness.md` | package/config/CI/contributor files | npm, exact dependency pair, CI and documentation maintenance |
