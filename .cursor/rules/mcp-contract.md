...
- Never treat a PAT as project-scoped. Preserve `project_id` query → `x-project-id` header → remembered `set_project` → `STUDIO_DEFAULT_PROJECT_ID` precedence.
- Never use token-only remembered state when a logical connector identity exists. Key remembered state by token + `connection_id`, `x-connection-id`, or client-supplied `mcp-session-id`; the compatibility PAT-global bucket is accepted only when identity is absent, and `set_project` must disclose that shared scope.
...
- Never imply this endpoint supplies `mcp-session-id`; it is accepted only as a caller-supplied connector identity.
